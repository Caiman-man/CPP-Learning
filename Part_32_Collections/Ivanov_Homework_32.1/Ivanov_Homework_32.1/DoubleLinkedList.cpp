...
DoubleLinkedList& DoubleLinkedList::operator++()
{
	...
	while (current != nullptr)
	{
		char* temp = new char[strlen(current->m_str) + 1];
		strcpy(temp, current->m_str);
		delete[] current->m_str;

		current->m_str = new char[strlen(temp) + 2];
		strcpy(current->m_str, temp);
		strcat(current->m_str, "!");
...
DoubleLinkedList& DoubleLinkedList::operator--()
{
	...
	while (current != nullptr)
	{
		if (strlen(current->m_str) > 0)
			current->m_str[strlen(current->m_str) - 1] = '\0';
...
//Operator << (file)
ostream& operator<< (ostream& osf, const DoubleLinkedList& source)
{
	fstream file("result.txt", ios::out);
	if (!file.fail())
	{
		if (source.m_first == nullptr)
			cout << "LIST IS EMPTY!\n";
...
//Operator >> (file)
istream& operator>> (istream& isf, DoubleLinkedList& source)
{
	string buf;
	
	fstream file("load.txt", ios::in);
	if (!file.fail())
	{
...
//Operator << (display)
//ostream& operator<< (ostream& os, const DoubleLinkedList& source)
//{
//	Element* current = source.m_first;
//	while (current != nullptr)
//	{
...
//Operator >> (display)
//istream& operator>> (istream& is, DoubleLinkedList& source)
//{
//	size_t count;
//	string buf;
