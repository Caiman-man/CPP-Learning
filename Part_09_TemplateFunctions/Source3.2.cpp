...
for (int i = 0; i < rowCount; ++i) {
        for (int j = 0; j < columnCount; ++j) {
            if (isMaxInRow(matr[i], matr[i][j], columnCount)) {
                if (isMinInColumn(matr, j, matr[i][j], rowCount)) {
                    haveSaddleItem = true;
                    std::cout << "Saddle item is: ";
                    std::cout << matr[i][j] << " (i=" << i << " j=" << j << ")" << std::endl;
                }
            }
        }
    }
...
bool isMaxInRow(const float *arr, const float &item, const unsigned &length) {
    for (int i = 0; i < length; ++i)
        if (arr[i] > item) return false;
    return true;
}

bool isMinInColumn(float **matr, const unsigned &columnIndex, const float &item, const unsigned &rowCount) {
    for (int i = 0; i < rowCount; ++i)
        if (item > matr[i][columnIndex]) return false;
    return true;
}